...
export function useAlgorithmVisualizer() {
  ...
  const [raceAlgorithms, setRaceAlgorithms] = useState<string[]>([
    "bubbleSort",
    "quickSort",
  ]);
  ...
  const [achievements, setAchievements] = useState<string[]>(() => {
    if (typeof window !== "undefined") {
      const savedAchievements = localStorage.getItem(
        "algorithmVisualizerAchievements"
      );
  ...
  const [showNewAchievement, setShowNewAchievement] = useState<string | null>(
    null
  );
  ...
  // Fix sound implementation in sorting algorithms
  // In the startSorting function, modify the sound implementation
  const startSorting = () => {
    if (!running && array.length > 0) {
      ...
      // Start sorting with continuous sound
    ...
    if (running) {
      ...
      if (raceMode && typeof window !== "undefined") {
        ...
        if (raceResults.length === 0) {
          setCurrentStep(
            "Race stopped. Click 'Start Race' to begin again or 'Reset' to reset the arrays."
          );
        }
      }
    }
    ...
    if (!running && array.length > 0) {
      ...
      try {
        ...
        if (!cancelRef.current) {
          ...
          newStats.fastestSort = Math.min(
            newStats.fastestSort,
            results[0].time
          );
          ...
          newStats.minComparisons = Math.min(
            newStats.minComparisons,
            minComparisons
          );

          setStats(newStats);
          localStorage.setItem(
            "algorithmVisualizerStats",
