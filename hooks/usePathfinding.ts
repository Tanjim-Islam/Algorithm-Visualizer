...
export function usePathfinding(soundEnabled: boolean) {
  ...
  const [finishNode, setFinishNode] = useState<{ row: number; col: number }>({
    row: 10,
    col: 35,
  });
    ...
    for (let i = 2; i < ROW_COUNT - 2; i++) {
      for (let j = 2; j < COL_COUNT - 2; j++) {
        if (
          ...
          !(
            Math.abs(i - finishNode.row) <= 2 &&
            Math.abs(j - finishNode.col) <= 2
          )
        ) {
  ...
  // Add sound implementation to pathfinding
  const animateAlgorithm = (
    visitedNodesInOrder: Node[],
    nodesInShortestPathOrder: Node[]
  ) => {
    const totalSteps =
      visitedNodesInOrder.length + nodesInShortestPathOrder.length;
    let step = 0;
    ...
    const animate = (timestamp: number) => {
      ...
      if (step < visitedNodesInOrder.length) {
        ...
        setProgress(Math.floor((step / totalSteps) * 100));
        setCurrentStep(
          `Building shortest path... (${pathIndex + 1}/${
