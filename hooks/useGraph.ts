...
export function useGraph(soundEnabled: boolean) {
    ...
    for (let i = 0; i < nodeCount; i++) {
      for (let j = 0; j < nodeCount; j++) {
        ...
        if (Math.random() < edgeProbability) {
          ...
          if (!isDirected && edges.some((e) => e.source === nodes[j].id && e.target === nodes[i].id)) {
            continue
          }
      ...
      if (
        !edges.some(
          (e) =>
            ...
            (!isDirected && e.source === nodes[i + 1].id && e.target === nodes[i].id),
        )
      ) {
        const weight = isWeighted ? Math.floor(Math.random() * 9) + 1 : 1
  ...
  const visualizeAlgorithm = () => {
    ...
    animateAlgorithm(visitedNodesInOrder, [], resultNodesInOrder, resultEdgesInOrder)
  }
  ...
  const animateAlgorithm = (
    ...
    const totalSteps =
      visitedNodesInOrder.length + activeEdgesInOrder.length + resultNodesInOrder.length + resultEdgesInOrder.length
    ...
    const animate = () => {
      ...
      if (visitedIdx < visitedNodesInOrder.length) {
        ...
        setCurrentStep(`Visiting node ${graph.nodes.find((n) => n.id === nodeId)?.label || nodeId}...`)

        if (soundEnabled) {
          const audioCtx = new (window.AudioContext || window.webkitAudioContext)()
          const oscillator = audioCtx.createOscillator()
          oscillator.type = "sine"
          oscillator.frequency.setValueAtTime(200 + visitedIdx * 20, audioCtx.currentTime)
        ...
        setCurrentStep(`Adding node ${graph.nodes.find((n) => n.id === nodeId)?.label || nodeId} to result...`)

        if (soundEnabled) {
          const audioCtx = new (window.AudioContext || window.webkitAudioContext)()
          const oscillator = audioCtx.createOscillator()
          oscillator.type = "sine"
          oscillator.frequency.setValueAtTime(300 + resultNodeIdx * 30, audioCtx.currentTime)
        ...
        if (soundEnabled) {
          ...
          const oscillator = audioCtx.createOscillator()
          oscillator.type = "sine"
          oscillator.frequency.setValueAtTime(400 + resultEdgeIdx * 40, audioCtx.currentTime)
        ...
        if (algorithm === "bfs" || algorithm === "dfs") {
          setCurrentStep(`${algorithm.toUpperCase()} traversal complete! Visited ${visitedNodes.length} nodes.`)
        } else if (algorithm === "topological") {
          setCurrentStep(`Topological sort complete! Found ordering of ${resultNodes.length} nodes.`)
        } else if (algorithm === "kruskal" || algorithm === "prim") {
          setCurrentStep(`Minimum Spanning Tree complete! MST contains ${resultEdges.length} edges.`)
        } else if (algorithm === "scc") {
          setCurrentStep(`Found ${resultNodes.length} strongly connected components.`)
        }
  ...
  // BFS algorithm
  const bfs = (graph: Graph, startNodeId: string, isDirected: boolean): string[] => {
  ...
  // DFS algorithm
  const dfs = (graph: Graph, startNodeId: string, isDirected: boolean): string[] => {
    const visitedNodesInOrder: string[] = []
    const visited = new Set<string>()
  ...
  const primMST = (graph: Graph, startNodeId: string): string[] => {
    ...
    const pq: { edgeId: string; weight: number; source: string; target: string }[] = []
    ...
    for (const edge of graph.edges) {
      if (edge.source === startNodeId || (!isDirected && edge.target === startNodeId)) {
        pq.push({
          edgeId: edge.id,
          weight: edge.weight,
          source: edge.source,
  ...
  const findSCC = (graph: Graph): string[] => {
    ...
    const fillOrder = (nodeId: string) => {
      ...
      // Get all adjacent nodes
      const adjacentNodes = graph.edges.filter((edge) => edge.source === nodeId).map((edge) => edge.target)
    ...
    const dfsUtil = (nodeId: string, component: Set<string>) => {
      ...
      const adjacentNodes = graph.edges.filter((edge) => edge.target === nodeId).map((edge) => edge.source)
